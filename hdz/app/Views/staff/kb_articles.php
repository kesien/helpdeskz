...
<div class="card mb-3">
    ...
    <div class="card-body">
        <div class="form-group">
            <label>
                <?php echo lang('Admin.form.view'); ?>
            </label>
            <select name="parent" class="form-control custom-select"
                onchange="if(this.value === ''){ location.href= '<?php echo site_url(route_to('staff_kb_articles')); ?>'; }else{location.href='<?php echo site_url(route_to('staff_kb_articles')); ?>/category/'+this.value;}">
                <option value="">
                    <?php echo lang('Admin.form.all'); ?>
                </option>
                <?php
                if (isset($kb_list)) {
                    $parent = 0;
    ...
    <table class="table table-hover table-striped">
        <thead>
            <tr>
                <th>
                    <?php echo lang('Admin.kb.article'); ?>
                </th>
                <th>
                    <?php echo lang('Admin.kb.author'); ?>
                </th>
                <th>
                    <?php echo lang('Admin.form.dateCreated'); ?>
                </th>
                <th>
        ...
        <tbody>
            ...
            if (count($articles_result) == 0) {
                ...
                foreach ($articles_result as $item) {
                    ...
                    <tr>
                        <td>
                            <?php echo '<a href="' . site_url(route_to('staff_kb_view_article', $item->id)) . '">' . resume_content($item->title, 50) . '</a>'; ?><br>
                            <small><i class="fa fa-folder-open-o"></i>
                        ...
                        <td>
                            <?php echo ($item->last_update == 0 ? '-' : dateFormat($item->last_update)); ?>
                        </td>
                        <td>
                            <?php echo $item->views; ?>
                        </td>
